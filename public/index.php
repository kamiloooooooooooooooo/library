...
$app->post('/user/register', function (Request $request, Response $response) use ($servername, $username, $password, $dbname) {
    ...
    if (!isset($data['username']) || !isset($data['password']) || empty($data['username']) || empty($data['password'])) {
        $response->getBody()->write(json_encode([
            "status" => "fail",
            "access_token" => null,
            "data" => ["title" => "Invalid input. Username and password are required."]
        ]));
        return $response->withStatus(400)->withHeader('Content-Type', 'application/json');
    }

    $uname = htmlspecialchars(trim($data['username']));
    $pass = htmlspecialchars(trim($data['password']));
    ...
    try {
        ...
        // Return error response with status code 500 for database errors
        $response->getBody()->write(json_encode([
            "status" => "fail",
            "access_token" => null,
            "data" => ["title" => "Database error: " . $e->getMessage()]
        ]));
